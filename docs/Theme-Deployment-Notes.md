...
**Scope note — the repo also holds a plugin with its own version line.** `plugin/` (Curtin P&C Order Columns) is *not* part of the theme: it releases as tag `plugin-vX.Y.Z`, GitHub Release title `Plugin vX.Y.Z`, commit prefix `Plugin vX.Y.Z:`. The theme uses tag `theme-vX.Y.Z`, Release title `Theme vX.Y.Z`, commit prefix `Theme vX.Y.Z:`. Both tag prefixes were introduced on 2026-07-29 — the theme's old bare `vX.Y.Z` tags and the plugin's `order-columns-v1.x` tags were renamed and no longer exist. Never bump one to match the other. Admin-only order/reporting tweaks belong in a plugin rather than `functions.php`, so they survive theme releases.
...
- **Release notes**: publish with `gh release create theme-vX.Y.Z` and attach the built zip. The **tag carries a `theme-` prefix and the release title a `Theme ` prefix** — tag `theme-vX.Y.Z`, title `Theme vX.Y.Z` (all historical theme releases were renamed and relabelled this way on 2026-07-29, so the Releases page reads unambiguously now that the plugin releases from the same repo). Plugin releases use tag `plugin-vX.Y.Z` and title `Plugin vX.Y.Z`. The `--notes` text should describe the change and call out whether assets were cache-bust renamed and whether it's been verified live yet.
